from .models import db, Release, Artist, Label, Tag
...
def total_releases():
    results = db.session.query(Release).count()
    return results


def total_artists():
    results = db.session.query(Artist).count()
    return results


def total_labels():
    results = db.session.query(Label).count()
    return results
...
def total_runtime():
    results = round(
        (
            (
                db.session.query(func.sum(Release.runtime))
                .scalar()
...
def top_rated_entities(model: db.Model,
                       sort_order: str = 'desc'):
    if model == Artist:
