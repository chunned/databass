...
from .models import db, Release, Label, Artist, Goal, Review, Tag
...
def delete(item_type: str, item_id: str):
    ...
    to_delete = db.session.query(model).where(model.id == item_id).one()
...
def get_artist_releases(artist_id: int):
    ...
    """
    releases = (
        db.session.query(Release, Label.name)
...
def get_label_releases(label_id: int):
    ...
    """
    releases = (
        db.session.query(Release, Artist)
        .join(Artist, Artist.id == Release.artist_id)
...
def get_distinct_col(table: db.Model,
                     col: str):
    """
...
def dynamic_artist_or_label_query(item_type: db.Model, filters: dict):
    """
